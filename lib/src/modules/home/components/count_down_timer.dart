...
class _CountDownTimerState extends State<CountDownTimer> {
  ...
  void initState() {
    super.initState();
    if (widget.settings['startDate'] != null && widget.settings['startDate'] != 0) {
      HomeLogic.to.startSelectedDate = DateTime.parse(widget.settings['startDate'].toString());

    } else {
      // Handle the case when startDate is null or 0
      // Provide a default value or alternative behavior
      HomeLogic.to.startSelectedDate = DateTime.now(); // For example, use the current date as a default value
    }

    (widget.settings['startDate'] != 0 && widget.settings['endDate'] != 0 && HomeLogic.to.startSelectedDate.isBefore(DateTime.now()))? HomeLogic.to.calculationFun(widget.settings):0;
  ...
  Widget timeCounter() {
    return Obx((){
      ...
      Color color = Color(int.parse(widget.settings["timerColor"].substring(1), radix: 16) + 0xFF000000);
      ...
      return (allZero) ? SizedBox(): GestureDetector(
        ...
        child: Padding(
          padding: widget.settings['margin'] == true? EdgeInsets.all(18.0):EdgeInsets.all(0),
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              (widget.settings['isTitleHidden'] == false) ? Padding(
                padding: EdgeInsets.symmetric(vertical: 6, horizontal: pageMarginHorizontal/1.5),
                child: Column(
                  children: [
                    ...
                    Container(
                      ...
                      child:
                      Text(
                        "${widget.settings['title']}", textAlign: widget.settings['titleAlignment'] == 'left' ? TextAlign.start: widget.settings['titleAlignment'] == "center"? TextAlign.center:TextAlign.end,   style: widget.settings['titleSize'] == "small"? context.text.titleSmall!.copyWith(color: AppColors.appTextColor):widget.settings['titleSize'] == "medium"? context.text.titleMedium!.copyWith(color: AppColors.appTextColor):context.text.titleLarge!.copyWith(color: AppColors.appTextColor),
                      ),
                    ),
                    widget.settings['image'] != null? (pageMarginVertical / 1.3).heightBox:(pageMarginVertical / 1.3).heightBox,
                  ],
                ),
              ) : const SizedBox(),

              (widget.settings["image"] != null && widget.settings["metadata"]["data"].isEmpty) ? singleImageVariant() : customImageVariant(context),
              ...
              // ):SizedBox(),
              Padding(
                padding: widget.settings['margin'] == true? EdgeInsets.only(left: 14.w, right: 14.w, bottom: 14.h): EdgeInsets.only(left: 6, top: 16, right: 6),
                child: Column(
                  children: [
                          ...
                          child: Column(
                            children: [
                              ...
                              Text(
                                "days",
                                textAlign: TextAlign.center,
                                style: context.text.bodyMedium!.copyWith(color: color, fontSize: 12.sp),
                              ),
                            ],
                          ),
                        ),
              ...
              (widget.settings['isSubTitleHidden'] == false) ? Padding(
                ...
                child: Container(
                  width: double.maxFinite,
                  child: Text(
                    "${widget.settings['subtitle']}", textAlign: TextAlign.center,   style: context.text.bodyLarge!.copyWith(color: color),
                  ),
                ),
              ) : const SizedBox(),
            ],
        ...
        children: [
          Padding(
            ...
            EdgeInsets.only(
                ...
                bottom: widget.settings['contentMargin'] == true ? pageMarginVertical : 0,
                top: widget.settings['contentMargin'] == true ? pageMarginVertical : 0
            )
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                borderRadius: widget.settings['margin'] == true? BorderRadius.circular(5.r):BorderRadius.circular(0),
              ),
              child: ClipRRect(
                borderRadius: widget.settings['margin'] == true? BorderRadius.circular(3.r):BorderRadius.circular(0),
                child: widget.settings['image'] != null? FadeInImage.memoryNetwork(
            ...
            children: [
              Padding(
                ...
                EdgeInsets.only(
                    ...
                    right: pageMarginHorizontal/1.5,
                    bottom: widget.settings['contentMargin'] == true ? pageMarginVertical : 0,
                    top: widget.settings['contentMargin'] == true ? pageMarginVertical : pageMarginHorizontal/1.5
                )
                    : widget.settings['contentMargin'] == true ? EdgeInsets.only(
                ...
                child: Stack(
                  ...
                  children: [
                      ...
                      child: ClipRRect(
                        ...
                        child: Stack(
                          children: [
                            ...
                            FadeInImage.memoryNetwork(
                              image:
                              widget.settings['image'] ?? "${productInfo?.image.split("?v=")[0]}?width=300}",
                              fit: BoxFit.cover,
                              height: widget.settings['displayType'] == "normal"? 300: widget.settings['displayType'] == "vertical"? 600: widget.settings['displayType'] == "auto"? null:230,
                              width: double.infinity,
                              imageErrorBuilder: (context, url,
                    ...
                    Positioned.fill(
                      child: Container(
                        width: double.maxFinite,
                        height: widget.settings['displayType'] == "normal" ? 300: widget.settings['displayType'] == "vertical"? 400: widget.settings['displayType'] == "auto"? null:230,
                        // height: 400,
                        decoration: BoxDecoration(
                          color: widget.settings['titlePosition'] == "center" ?  Colors.black.withOpacity(0.4):null,
                          borderRadius: widget.settings['margin'] == true? BorderRadius.circular(5.r):BorderRadius.circular(0),
                          // colorBlendMode: settings['titlePosition'] == "center" ? BlendMode.darken:null,
                        ),
                      ),
              ...
              Positioned(
                  ...
                  top: 0,
                  bottom: 0,
                  child: widget.settings['titlePosition'] == "center" ? Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    crossAxisAlignment: CrossAxisAlignment.center,
                    children: [
  ...
  Widget build(BuildContext context) {
    ...
    return (widget.settings['startDate'] != 0 && widget.settings['endDate'] != 0 && HomeLogic.to.startSelectedDate.isBefore(DateTime.now()))? timeCounter():SizedBox();
  }
}