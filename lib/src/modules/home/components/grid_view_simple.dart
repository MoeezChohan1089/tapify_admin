...
class ProductGridViewSimple extends StatelessWidget {
  ...
  ProductGridViewSimple({Key? key, required this.settings})
      : super(key: key);
    ...
    if (size == 'normal') {
      if (viewType == 'small') {
        ...
        if (isTitleHidden && showAddToCartButton && isPriceHidden) {
          ...
          return settings["contentMargin"] ?  .56 : 0.58; //----- when image + title + price
        } else if (isTitleHidden && showAddToCartButton && !isPriceHidden) {
          return 0.64; //------ when image + price +  button
        } else if (!isTitleHidden && showAddToCartButton && isPriceHidden) {
          return 0.64; //---- when image + title + button
        } else if (isTitleHidden && !showAddToCartButton && !isPriceHidden) {
          return settings["contentMargin"] ? .7 : 0.8; //----- when only price and image
        } else if (!isTitleHidden && !showAddToCartButton && isPriceHidden) {
          return settings["contentMargin"] ? 0.65 : 0.7; //------ when only title and image
        } else if (isTitleHidden && !showAddToCartButton && isPriceHidden) {
        ...
        if (isTitleHidden && showAddToCartButton && isPriceHidden) {
          return settings["contentMargin"] ? .76 : 0.76; //----- when image + button
        } else if (!isTitleHidden && showAddToCartButton && !isPriceHidden) {
          return 0.55; //------- when only image + title + price + button
        } else if (!isTitleHidden && !showAddToCartButton && !isPriceHidden) {
          return settings["contentMargin"] ? .5 : 0.55; //----- when image + title + price
        } else if (isTitleHidden && showAddToCartButton && !isPriceHidden) {
          ...
          return settings["contentMargin"] ? .6 : 0.65; //------ when only title and image
        } else if (isTitleHidden && !showAddToCartButton && isPriceHidden) {
  ...
  widgetBuilder(BuildContext context, ProductInfo? productInfo,
      dynamic indexData) {
    return GestureDetector (
      ...
      child: Column(
        children: [
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                border: settings["hideBorders"]
                    ? null
                    : Border.all(
                    color: AppColors.appBordersColor, width: 0.8),
                borderRadius: BorderRadius.only(
                  topLeft: Radius.circular(3.r),
                  topRight: Radius.circular(3.r),
                  bottomLeft: settings["showAddToCart"] ? Radius.circular(0.r): Radius.circular(3.r),
                  bottomRight: settings["showAddToCart"] ? Radius.circular(0.r): Radius.circular(3.r),
                ),
              ),
              child: ClipRRect(
                borderRadius: BorderRadius.only(
                ...
                child: productInfo!.image.isNotEmpty?  FadeInImage.memoryNetwork(
          ...
          settings["showAddToCart"]? GestureDetector(
            onTap: () {
              if(productInfo.availableForSale == true){
                HapticFeedback.lightImpact();
                ProductSheet.to.productDetailBottomSheet(context: context, fetchProductDetail: true, productInfo: productInfo);
              }
            },
            child: Container(
              height: 28.h,
              decoration: BoxDecoration(
                color:  productInfo.availableForSale == true ? AppConfig.to.quickViewBGColor.value : AppColors.appHintColor,
                borderRadius: BorderRadius.only(
              ...
              child: Padding(
                ...
                child: productInfo.availableForSale == true ? Row(
            ...
            children: [
              ...
              Expanded(
                ...
                Row(
                  children: [
                    Text(
                      productInfo.price == 0 ? "FREE" : CurrencyController.to.getConvertedPrice(
                          priceAmount: productInfo.price),
                      maxLines: 1,
                      style: context.text.bodyMedium?.copyWith(
                          color: productInfo.compareAtPrice != 0? AppColors.appPriceRedColor:AppColors.appTextColor,
                          letterSpacing: .01,
                          height: .5
                      ),
                    ),
              ...
              settings['viewType'] == 'large'?   Obx(() {
                ...
                return GestureDetector(
                  onTap: () {
                    ...
                    WishlistLogic.to.getProductFromID(context: context, productID: productInfo.id);
                  ...
                  child: Padding(
                    ...
                    child: settings['viewType'] != 'small'? Align(
              ...
              Expanded(
                child: Container(
                  ...
                  child: Text(
                    ...
                    productInfo!.title,
                    maxLines: settings["hideContentPrice"]
                        ? 2:1,
              ...
              settings["hideContentPrice"]
                ...
                return GestureDetector(
                  ...
                  child: Padding(
                    ...
                    height: 20,
                      child: settings['viewType'] == 'large'? Align(
                        alignment: Alignment.topRight,
                        child: SvgPicture.asset(
                          isProductInWishlist
                              ? Assets.icons.heartFilled
