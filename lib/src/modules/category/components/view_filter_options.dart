...
class _FilterOptionsViewState extends State<FilterOptionsView> {
  ...
  setThePriceRange() {
    ...
    if (existingIndex != -1) {
      priceMinTxtController.text = CategoryLogic
          .to.selectedFilters[existingIndex]["price"]["min"]
          .toString();
      priceMaxTxtController.text = CategoryLogic
          .to.selectedFilters[existingIndex]["price"]["max"]
          .toString();
      // priceMax
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: GetBuilder<CategoryLogic>(builder: (categoryLogic) {
          return widget.filter.id == "filter.v.price"
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: FlutterSlider(
                              values: [
                                double.parse(priceMinTxtController.text),
                                double.parse(priceMaxTxtController.text)
                              ],
                              rangeSlider: true,
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          Text(
                            "to",
                            style: context.text.bodyMedium?.copyWith(height: 3),
                          ),
                          20.widthBox,
                          Expanded(
                              child: Column(
              ...
              : SingleChildScrollView(
                  child: Column(
                    children:
                        List.generate(widget.filter.values.length, (index) {
                      final Map<String, dynamic> input =
                          jsonDecode(widget.filter.values[index].input);
                        ...
                        children: [
                          InkWell(
                            onTap: () {
                              ...
                              print(
                                  "before Filters List values => ${categoryLogic.selectedFilters}");
                              ...
                              try {
                                ...
                                existingFilter =
                                    categoryLogic.selectedFilters.firstWhere(
                                  (f) => const DeepCollectionEquality()
                                      .equals(f, input),
                                );
                              } catch (e) {
                                existingFilter =
                                    null; // Set a default value or do any other necessary handling
                              }

                              print(
                                  "value of existing filter is $existingFilter");

                              if (existingFilter != null) {
                                // If the filter already exists, remove it from the list
                                categoryLogic.selectedFilters
                                    .remove(existingFilter);
                              } else {
                                // If the filter doesn't exist, add it to the list
                                categoryLogic.selectedFilters.add(jsonDecode(
                                    widget.filter.values[index].input));
                              }

                              print(
                                  "=> Current Filters List values => ${categoryLogic.selectedFilters}");
                            ...
                            child: Container(
                              ...
                              child: Row(
                                ...
                                children: [
                                  SizedBox(
                                    width: 30.w,
                                    child: categoryLogic.selectedFilters
                                                .indexWhere(
                                              (f) =>
                                  ...
                                  Expanded(
                                      flex: 2,
                                      child: Text(
                                        "${widget.filter.values[index].label}  (${widget.filter.values[index].count})"
                                            .capitalize!,
        ...
        bottomNavigationBar: SafeArea(
          child: Padding(
            padding: EdgeInsets.only(bottom: pageMarginVertical),
...
// The new price object to be added
                  ...
                  int existingIndex = -1;
                  for (int i = 0;
                      i < CategoryLogic.to.selectedFilters.length;
                      i++) {
                    Map<String, dynamic> filter =
                        CategoryLogic.to.selectedFilters[i];
                    if (filter.containsKey("price")) {
                      existingIndex = i;
                      break;
                    }
                ...
                print(
                    "current filters list is ${CategoryLogic.to.selectedFilters}");
